...
type ContributionGraphContextValue = {
  ...
  cellRadius: number;
  gap: number;
  colors: { empty: string; level1: string; level2: string; level3: string; level4: string };
...
const ContributionGraphContext = React.createContext<ContributionGraphContextValue | null>(null);

function useContributionGraph() {
  const context = React.useContext(ContributionGraphContext);
  if (!context) {
    throw new Error("useContributionGraph must be used within a ContributionGraph");
  }
  return context;
}
...
function buildGrid(
  ...
  const contributionsMap = new Map<string, ContributionData>(data.map((entry) => [entry.date, entry]));
  ...
  const totalDays = Math.floor((gridEnd.getTime() - gridStart.getTime()) / MS_IN_DAY) + 1;
  ...
  for (let w = 0; w < totalWeeks; w++) {
    ...
    for (let d = 0; d < 7; d++) {
      ...
      const level = withinRange ? entry?.level ?? calculateLevel(count, maxCount) : 0;
...
function ContributionGraph({
  ...
  const dataMap = React.useMemo(() => new Map(data.map((d) => [d.date, d])), [data]);
  ...
  const colors = React.useMemo(
    () => ({ empty: colorEmpty, level1: colorLevel1, level2: colorLevel2, level3: colorLevel3, level4: colorLevel4 }),
  ...
  const contextValue = React.useMemo(
    ...
    [dataMap, matrix, headers, weeks, cellSize, cellRadius, gap, colors, tooltip, formatDate, formatCount],
  );
...
interface ContributionGraphGridProps extends React.HTMLAttributes<HTMLDivElement> {
...
function ContributionGraphGrid({
  ...
  const { weeksMatrix, monthHeaders, totalWeeks, cellSize, cellRadius, gap, colors, setTooltip } =
  ...
  const handleMouseLeave = React.useCallback(() => setTooltip(null), [setTooltip]);

  const levelColors = [colors.empty, colors.level1, colors.level2, colors.level3, colors.level4];

  return (
    <div ref={containerRef} className={cn("overflow-x-auto", className)} {...props}>
        ...
        {showMonthLabels && (
          <div
            ...
            style={{
              ...
              gap: `${gap}px`,
              gridTemplateColumns: monthHeaders.map((h) => `repeat(${h.weeks}, 1fr)`).join(" "),
            }}
          >
            {monthHeaders.map((header, i) => (
              <div key={`${header.label}-${i}`} className="truncate" style={{ gridColumn: `span ${header.weeks}` }}>
              ...
              week.map((cell, dayIndex) => (
                <div
                  ...
                  className={cn(
                    ...
                    cell.withinRange && "cursor-pointer hover:ring-2 hover:ring-ring",
                  )}
                  style={{
                    backgroundColor: cell.withinRange ? levelColors[cell.level] : "transparent",
...
interface ContributionGraphTooltipProps extends Omit<React.HTMLAttributes<HTMLDivElement>, "children"> {
  children?: (data: { date: string; count: number }) => React.ReactNode;
}

function ContributionGraphTooltip({ children, className, ...props }: ContributionGraphTooltipProps) {
    ...
    >
      {children ? (
        ...
        <div className="whitespace-nowrap">
          <div className="font-medium text-popover-foreground">{formatCount(tooltip.count)}</div>
...
interface ContributionGraphLegendProps extends React.HTMLAttributes<HTMLDivElement> {
  lessLabel?: string;
  moreLabel?: string;
}
...
function ContributionGraphLegend({
  ...
  return (
    <div
      className={cn("flex items-center justify-end gap-1.5 text-[10px] text-muted-foreground", className)}
      {...props}
    >
      <span>{lessLabel}</span>
      {levelColors.map((color, i) => (
        <div key={i} className="size-2.5 sm:size-3" style={{ backgroundColor: color, borderRadius: `${cellRadius}px` }} />
      ))}
      <span>{moreLabel}</span>
    </div>
  );
