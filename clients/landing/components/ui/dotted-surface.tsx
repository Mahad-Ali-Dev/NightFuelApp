...
export function DottedSurface({ className, ...props }: DottedSurfaceProps) {
	...
	useEffect(() => {
		...
		// Lime dots in dark, muted-ink dots in light.
		const dark = resolvedTheme === 'dark';
		const cr = dark ? 168 : 60, cg = dark ? 204 : 70, cb = dark ? 60 : 60;
		...
		const animate = () => {
			...
			for (let ix = 0; ix < AMOUNTX; ix++) {
				for (let iy = 0; iy < AMOUNTY; iy++) {
					const index = i * 3;
					posArr[index + 1] = Math.sin((ix + count) * 0.3) * 50 + Math.sin((iy + count) * 0.5) * 50;
					i++;
				}
			}
