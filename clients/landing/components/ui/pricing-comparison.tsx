    ...
    <div className="w-full">
      <div className="flex text-center justify-center items-center gap-4 flex-col">
        ...
        <div className="grid text-left w-full grid-cols-3 lg:grid-cols-4 divide-x divide-[var(--color-border)] pt-14 border border-[var(--color-border)] rounded-2xl overflow-hidden">
          ...
          <div className="px-4 py-5 md:px-6 gap-2 flex flex-col">
            <p className="text-xl">Free apps</p>
            <p className="text-sm text-muted-foreground">MyFitnessPal, Noom &amp; co.</p>
            <p className="flex items-baseline gap-1 text-xl mt-6"><span className="text-3xl">$0</span></p>
            <Button variant="ghost" className="gap-2 mt-6" disabled>Blind to your clock</Button>
          </div>

          <div className="px-4 py-5 md:px-6 gap-2 flex flex-col">
            <p className="text-xl">Zeitra Monthly</p>
            <p className="text-sm text-muted-foreground">Full access, month to month.</p>
            <p className="flex items-baseline gap-1 text-xl mt-6"><span className="text-4xl">$9.99</span><span className="text-sm text-muted-foreground">/mo</span></p>
            <Button variant="secondary" className="gap-2 mt-6">Start free trial <MoveRight className="w-4 h-4" /></Button>
          </div>

          <div className="px-4 py-5 md:px-6 gap-2 flex flex-col relative rounded-xl bg-[var(--color-lime)]/[0.06] ring-1 ring-[var(--color-lime)]/40">
            <div className="flex items-center gap-2">
              <p className="text-xl">Zeitra Annual</p>
              <Badge className="text-[10px]">Best value</Badge>
            </div>
            <p className="text-sm text-muted-foreground">Founding member — save ~60%.</p>
            <p className="flex items-baseline gap-1 text-xl mt-6"><span className="text-4xl">$59</span><span className="text-sm text-muted-foreground">/yr</span></p>
            <Button className="gap-2 mt-6">Start free trial <MoveRight className="w-4 h-4" /></Button>
          </div>
