...
const AI_LEVELS = [
    {
        ...
        icon: 'flash',
        color: '#7C4DFF' // Purple coach color
    },
    {
        value: 'medium',
        ...
        <View style={styles.container}>
            <ScrollView contentContainerStyle={{ padding: spacing.xl }}>
                <Text style={[typography.display, { color: colors.text.primary, marginBottom: spacing.sm }]}>
                    AI <Text style={{ color: colors.accent.purple || '#7C4DFF' }}>Optimization</Text>
                </Text>
                ...
                <View style={{ gap: spacing.md }}>
                    {AI_LEVELS.map((level) => {
                        ...
                        return (
                            <TouchableOpacity key={level.value} onPress={() => setAiLevel(level.value as any)} activeOpacity={0.8}>
                                <Card
                                    variant={isSelected ? 'elevated' : 'default'}
                                    style={[
                                        styles.levelCard,
                                        isSelected && { borderColor: level.color, borderWidth: 1 }
                                    ]}
                                >
                                    <View style={styles.cardHeader}>
                                        <View style={[styles.iconBox, { backgroundColor: `${level.color}20` }]}>
            ...
            <View style={[styles.footer, { paddingHorizontal: spacing.xl, paddingBottom: spacing['2xl'] }]}>
                <Button
                    ...
                    iconRight={<Ionicons name="sparkles" size={20} color="#fff" />}
                    onPress={handleNext}
                    fullWidth
                />
            </View>
