...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, ActivityIndicator, TextInput, Alert, RefreshControl, Modal, FlatList } from 'react-native';
...
export default function RoutinesScreen() {
    ...
    const createM = useMutation({
        ...
        onSuccess:()=>{ qc.invalidateQueries({queryKey:['workout-routines']}); setShowCreate(false); setNewName(''); setSelEx([]); },
        onError:(err:any)=>Alert.alert('Error', err?.response?.data?.message??'Failed to create.'),
    });
    const addEx = (ex:Exercise) => { if(selEx.find(e=>e.name===ex.name)) return; setSelEx([...selEx,{name:ex.name,sets:3,reps:10}]); setShowPicker(false); };
    return (
        <View style={[s.container,{backgroundColor:colors.background.primary}]}>
            <View style={[s.header,{paddingTop:insets.top+16,borderBottomColor:colors.border.default}]}>
                ...
                <TouchableOpacity style={[s.addBtn,{backgroundColor:colors.accent.coral}]} onPress={()=>setShowCreate(true)}><Ionicons name="add" size={20} color="#FFF" /></TouchableOpacity>
            </View>
            {/* AI Planner Banner */}
            <TouchableOpacity style={[s.aiBanner,{marginHorizontal:20,marginTop:16,backgroundColor:withAlpha('#A855F7',0.1),borderColor:withAlpha('#A855F7',0.3)}]} onPress={()=>router.push('/(exercises)/ai-planner' as any)} activeOpacity={0.85}>
                <View style={[s.aiIcon,{backgroundColor:withAlpha('#A855F7',0.2)}]}><Ionicons name="sparkles" size={22} color="#A855F7" /></View>
                <View style={{flex:1,marginLeft:14}}>
                    <Text style={[typography.subhead,{color:'#A855F7',fontWeight:'800'}]}>Generate with Ria AI</Text>
                    <Text style={[typography.caption,{color:colors.text.tertiary,marginTop:2}]}>Let Coach Ria build a personalized routine for your goals</Text>
                </View>
            ...
            <ScrollView contentContainerStyle={{paddingBottom:100,paddingTop:12}} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={isLoadingRoutines} onRefresh={()=>qc.invalidateQueries({queryKey:['workout-routines']})} tintColor={colors.accent.coral} />}>
                {(routines.length>0||isLoadingRoutines)&&(
                    <View style={{padding:20}}>
                        ...
                        {isLoadingRoutines?<ActivityIndicator color={colors.accent.coral} />:routines.map((r:any,idx:number)=>{
                            const c=[colors.accent.coral,colors.accent.cyan,colors.accent.emerald,colors.accent.purple][idx%4]!;
                            return (
                                <TouchableOpacity key={r.id||idx} style={[s.myCard,{backgroundColor:colors.background.secondary,borderColor:colors.border.default}]} onPress={()=>router.push({pathname:'/training/onboarding',params:{routineId:r.id}})} activeOpacity={0.85}>
                                    ...
                                    <View style={{flex:1,paddingLeft:16}}>
                                        ...
                                        <Text style={[typography.caption,{color:colors.text.tertiary,marginTop:2}]}>{r.exercises?.length||0} exercises</Text>
                                    </View>
                                    <TouchableOpacity style={[s.startBtn,{backgroundColor:c}]} onPress={()=>router.push({pathname:'/training/workout',params:{routineId:r.id}})}>
                ...
                {!isLoadingRoutines&&routines.length===0&&(
                    <View style={{paddingHorizontal:20,paddingTop:8}}>
                        <TouchableOpacity style={[s.createPrompt,{borderColor:colors.accent.coral,backgroundColor:withAlpha(colors.accent.coral,0.05)}]} onPress={()=>setShowCreate(true)}>
                            <Ionicons name="add-circle-outline" size={28} color={colors.accent.coral} />
                            <Text style={[typography.subhead,{color:colors.accent.coral,fontWeight:'bold',marginTop:8}]}>Create Your First Routine</Text>
                        </TouchableOpacity>
                    </View>
                )}
            </ScrollView>
            <Modal visible={showCreate} animationType="slide" transparent>
                <View style={[s.createOverlay,{backgroundColor:colors.background.primary,paddingTop:insets.top}]}>
                    <View style={[s.createHeader,{borderBottomColor:colors.border.default}]}>
                        <TouchableOpacity onPress={()=>setShowCreate(false)}><Text style={[typography.body,{color:colors.text.tertiary}]}>Cancel</Text></TouchableOpacity>
                        <Text style={[typography.heading,{color:colors.text.primary}]}>New Routine</Text>
                        <TouchableOpacity onPress={()=>createM.mutate()} disabled={!newName||selEx.length===0||createM.isPending}><Text style={[typography.body,{color:(!newName||selEx.length===0)?colors.text.tertiary:colors.accent.coral,fontWeight:'bold'}]}>Save</Text></TouchableOpacity>
                    </View>
                    <ScrollView style={{flex:1,padding:20}}>
                        ...
                        <View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center',marginTop:28,marginBottom:16}}>
                            ...
                            <TouchableOpacity onPress={()=>setShowPicker(true)}><Text style={[typography.caption,{color:colors.accent.cyan,fontWeight:'bold'}]}>+ ADD</Text></TouchableOpacity>
                        </View>
                        {selEx.map((ex,idx)=>(<View key={idx} style={[s.selRow,{backgroundColor:colors.background.secondary,borderColor:colors.border.default}]}><View style={{flex:1}}><Text style={[typography.subhead,{color:colors.text.primary,fontWeight:'bold'}]}>{ex.name}</Text><Text style={[typography.caption,{color:colors.text.tertiary}]}>{ex.sets}x{ex.reps}</Text></View><TouchableOpacity onPress={()=>setSelEx(selEx.filter(e=>e.name!==ex.name))}><Ionicons name="trash-outline" size={20} color={colors.accent.coral} /></TouchableOpacity></View>))}
                    </ScrollView>
                </View>
            </Modal>
                ...
                <View style={[s.pickerOverlay,{backgroundColor:'rgba(0,0,0,0.8)'}]}>
                    <View style={[s.pickerContent,{backgroundColor:colors.background.primary}]}>
                        <View style={[s.pickerHead,{borderBottomColor:colors.border.default}]}>
                            ...
                            <TouchableOpacity onPress={()=>setShowPicker(false)}><Ionicons name="close" size={24} color={colors.text.primary} /></TouchableOpacity>
                        </View>
                        ...
                        {isSrch?<ActivityIndicator color={colors.accent.coral} style={{marginTop:20}} />:(
                            <FlatList data={sr} keyExtractor={(i)=>i.id} renderItem={({item})=>(
                                <TouchableOpacity style={[s.pickerItem,{borderBottomColor:colors.border.default}]} onPress={()=>addEx(item)}>
                                    <View style={{flex:1}}><Text style={[typography.subhead,{color:colors.text.primary,fontWeight:'bold'}]}>{item.name}</Text><Text style={[typography.caption,{color:colors.text.tertiary}]}>{item.muscleGroup}</Text></View>
                                    <Ionicons name="add-circle" size={24} color={colors.accent.cyan} />
                                </TouchableOpacity>
                            )} ListEmptyComponent={<View style={{padding:40,alignItems:'center'}}><Text style={[typography.caption,{color:colors.text.tertiary}]}>{sq.length>1?'No results':'Start typing...'}</Text></View>} />
                        )}
                    </View>
                </View>
            </Modal>
...
const s = StyleSheet.create({
    ...
    createPrompt:{borderWidth:1.5,borderStyle:'dashed',borderRadius:20,padding:32,alignItems:'center'},
