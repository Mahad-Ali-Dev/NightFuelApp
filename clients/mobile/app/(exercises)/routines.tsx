...
import { RoutinePressable } from '@/components/RoutinePressable';
export default function RoutinesScreen() {
    ...
    // Per-chip text resolver: ink on the brand lime (coral/coralDark), white on
    // the cyan/emerald/purple chips. White-on-lime is a brand violation.
    const onColorText = (c:string) => (c===colors.accent.coral||c===colors.accent.coralDark) ? colors.text.inverse : '#FFF';
    // Athletic header summary — total routines + their combined exercise count.
    const totalRoutines = routines.length;
    const totalExercises = routines.reduce((sum:number,r:any)=>sum+(r.exercises?.length||0),0);
    ...
    return (
        <View style={[s.container,{backgroundColor:colors.background.primary}]}>
            ...
            <Animated.View entering={FadeInDown.delay(0).springify()} style={[s.header,{paddingTop:insets.top+16,borderBottomColor:colors.border.default}]}>
                <RoutinePressable hitSlop={headerHitSlop} accessibilityRole="button" accessibilityLabel="Go back" onPress={()=>router.back()}><Ionicons name="arrow-back" size={24} color={colors.text.primary} /></RoutinePressable>
                <Text style={[typography.h2,{color:colors.text.primary}]}>Workout Routines</Text>
                <RoutinePressable hitSlop={headerHitSlop} accessibilityRole="button" accessibilityLabel="Add" style={shadows.glow(colors.accent.coral)} onPress={()=>setShowCreate(true)}>
                    <LinearGradient colors={colors.gradients.coral} start={{x:0,y:0}} end={{x:1,y:1}} style={s.addBtn}><Ionicons name="add" size={20} color={colors.text.inverse} /></LinearGradient>
                </RoutinePressable>
            </Animated.View>
            {/* AI Planner Banner */}
            <Animated.View entering={FadeInDown.delay(40).springify()}>
                <RoutinePressable accessibilityRole="button" accessibilityLabel="Generate with Ria AI" style={[s.aiBanner,{marginHorizontal:20,marginTop:16,backgroundColor:withAlpha(colors.accent.purple,0.1),borderColor:withAlpha(colors.accent.purple,0.3)}]} onPress={()=>router.push('/(exercises)/ai-planner' as any)}>
            ...
            <ScrollView contentContainerStyle={{paddingBottom:100,paddingTop:12}} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={isLoadingRoutines} onRefresh={()=>qc.invalidateQueries({queryKey:['workout-routines']})} tintColor={colors.accent.coral} />}>
                {(routines.length>0||isLoadingRoutines)&&(
                    <View style={{padding:20}}>
                        {!isLoadingRoutines&&totalRoutines>0&&(
                            <Animated.View entering={FadeInDown.delay(80).springify()} style={[s.summaryRow,{backgroundColor:colors.background.secondary,borderColor:colors.border.default}]}>
                                <View style={s.summaryStat}>
                                    <Text style={[typography.statMedium,{color:colors.accent.coral}]}>{totalRoutines}</Text>
                                    <Text style={[typography.overline,{color:colors.text.secondary}]}>{totalRoutines===1?'Routine':'Routines'}</Text>
                                </View>
                                <View style={[s.summaryDivider,{backgroundColor:colors.border.default}]} />
                                <View style={s.summaryStat}>
                                    <Text style={[typography.statMedium,{color:colors.text.primary}]}>{totalExercises}</Text>
                                    <Text style={[typography.overline,{color:colors.text.secondary}]}>Exercises</Text>
                                </View>
                            </Animated.View>
                        )}
                        ...
                        {isLoadingRoutines?[0,1,2].map((i)=>(<Skeleton key={i} width="100%" height={68} radius={borderRadius.xl} style={{marginBottom:12}} />)):routines.map((r:any,idx:number)=>{
                            ...
                            const startText=onColorText(c);
                            return (
                                <Animated.View key={r.id||idx} entering={FadeInDown.delay(120+idx*40).springify()}>
                                    <RoutinePressable accessibilityRole="button" accessibilityLabel={r.name||r.title} style={[s.myCard,{backgroundColor:colors.background.secondary,borderColor:colors.border.default}]} onPress={()=>router.push({pathname:'/training/onboarding',params:{routineId:r.id}})}>
                                        ...
                                        <View style={{flex:1,paddingLeft:16}}>
                                            ...
                                            <View style={s.countRow}>
                                                <Text style={[typography.statSmall,{color:colors.text.primary}]}>{r.exercises?.length||0}</Text>
                                                <Text style={[typography.overline,{color:colors.text.secondary,marginLeft:6}]}>Exercises</Text>
                                            </View>
                                        </View>
                                        <RoutinePressable hitSlop={{top:10,bottom:10,left:10,right:10}} accessibilityRole="button" accessibilityLabel={`Start ${r.name||r.title}`} style={[s.startBtn,{backgroundColor:c}]} onPress={()=>router.push({pathname:'/training/workout',params:{routineId:r.id}})}>
                                            <Text style={[typography.overline,{color:startText}]}>START</Text>
                                        </RoutinePressable>
                                    </RoutinePressable>
                                </Animated.View>
                            );
                ...
                <View style={[s.createOverlay,{backgroundColor:colors.background.primary,paddingTop:insets.top}]}>
                    ...
                    <ScrollView style={{flex:1}} contentContainerStyle={{padding:20,paddingBottom:insets.bottom+24}}>
                        ...
                        <View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center',marginTop:28,marginBottom:16}}>
                            ...
                            <TouchableOpacity hitSlop={{top:12,bottom:12,left:12,right:12}} accessibilityRole="button" accessibilityLabel="Add exercise" style={s.addExBtn} onPress={()=>setShowPicker(true)}><Text style={[typography.overline,{color:colors.accent.cyan}]}>+ Add</Text></TouchableOpacity>
                        </View>
            ...
            <Modal visible={showPicker} animationType="slide" transparent>
                <View style={[s.pickerOverlay,{backgroundColor:withAlpha(colors.background.primary,0.8)}]}>
                    <View style={[s.pickerContent,{backgroundColor:colors.background.primary}]}>
                        ...
                        {isSrch?<View style={{marginTop:8}}>{[0,1,2,3,4].map((i)=>(
                            ...
                            <FlatList data={sr} keyExtractor={pickerKeyExtractor} renderItem={renderPickerItem} removeClippedSubviews={Platform.OS === 'android'} initialNumToRender={10} maxToRenderPerBatch={10} windowSize={7} contentContainerStyle={{paddingBottom:insets.bottom+24}} ListEmptyComponent={<View style={{padding:40,alignItems:'center'}}><Text style={[typography.caption,{color:colors.text.secondary}]}>{sq.length>1?'No results':'Start typing...'}</Text></View>} />
                        )}
                    </View>
                </View>
            </Modal>
...
const s = StyleSheet.create({
    ...
    colorBar:{width:4,alignSelf:'stretch'}, startBtn:{paddingHorizontal:16,minHeight:44,borderRadius:20,marginRight:16,alignItems:'center',justifyContent:'center'},
    countRow:{flexDirection:'row',alignItems:'baseline',marginTop:2},
    summaryRow:{flexDirection:'row',alignItems:'center',borderRadius:16,borderWidth:1,paddingVertical:16,marginBottom:20},
    summaryStat:{flex:1,alignItems:'center'}, summaryDivider:{width:1,height:36,opacity:0.6},
    addExBtn:{minHeight:44,justifyContent:'center'},
