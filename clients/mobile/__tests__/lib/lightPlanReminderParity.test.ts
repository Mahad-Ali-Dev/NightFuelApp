 ...
 *
 * This suite pins that relationship BIT-IDENTICALLY:
 *
 ...
 *
 * `buildShiftReminders` deliberately computes `nf-avoid-light` via direct Date
 * arithmetic (end + (sleepStartAfterEnd − BLUE_BLOCKER_LEAD_HOURS)h) rather than
 * calling `computeLightPlan`, to keep the function pure (no throw on malformed
 * ISO). These equalities are the lock that guarantees the two derivations stay
 * equal by VALUE — if anyone tweaks the blue-blocker lead, the OFFSETS, or the
 * brightLightWindow in EITHER module without the other, this suite turns RED.
 *
 * Both `buildShiftReminders` and `computeLightPlan` are pure (no React / native /
 * network / Date.now), so this needs no mocks. Timestamps are UTC so the
