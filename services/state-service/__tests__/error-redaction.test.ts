/**
 * Regression suite — state-service routes through the SHARED Fastify error
 ...
 *   - state-service runs on Fastify 5; the inject() / setErrorHandler API used
 *     here is unchanged across v4 → v5, so the test shape stays identical to
 *     its v4 siblings (exercise-service / plan-service / etc.).
...
describe('state-service shared error handler — 500 leak redaction (registerFastifyErrorHandler)', () => {
