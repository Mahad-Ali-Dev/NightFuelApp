/**
 * Regression suite — decision-engine routes through the SHARED Fastify error
 ...
 *   - decision-engine runs on Fastify 5; the inject() / setErrorHandler API
 *     used here is unchanged across v4 → v5, so the test shape stays identical
 *     to its v4 siblings (exercise-service / plan-service / etc.).
 *   - decision-engine's package.json sets `test: vitest run`, but this suite
 *     uses no vitest-specific globals — only describe / it / expect /
 *     beforeAll / afterAll — so it runs unchanged under either vitest or
 *     jest, matching every other per-service error-redaction suite.
 ...
 *         /bad-internal test.
 */
// decision-engine runs vitest WITHOUT globals, so the jest-style describe/it/expect
// must be imported explicitly (the other services run jest, which injects globals).
...
describe('decision-engine shared error handler — 500 leak redaction (registerFastifyErrorHandler)', () => {
